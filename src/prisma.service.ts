...
export class PrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  ...
  constructor() {
    ...
    const enableSqlLogging =
      process.env.SQL_LOGGING === 'true' || process.env.NODE_ENV === 'development'
    ...
    try {
      ...
      if (url.startsWith('postgres') && url.includes('pooler.supabase.com')) {
        ...
        if (isPoolerPort && !hasPgBouncerParam) {
          this.logger.warn(
            'Detected Supabase Pooler (port 6543) without pgbouncer=true. Add ?pgbouncer=true&connection_limit=1 to DATABASE_URL to avoid prepared statement errors (42P05).'
          )
        }
      }
    } catch (e) {
   ...
   */
  async executeWithErrorHandling<T>(operation: () => Promise<T>, context: string): Promise<T> {
    try {
