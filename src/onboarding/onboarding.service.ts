    ...
    try {
      ...
      if ((error as any)?.code === 'P2002') {
        ...
        if (target?.includes('name')) {
          throw new ConflictException(
            `A ${target.includes('tenant') ? 'company' : 'workspace'} with this name already exists. Please choose a different name.`
          )
        }
      ...
      if (message.includes('prepared statement') || message.includes('already exists')) {
        this.logger.error(
          'Detected PgBouncer prepared statement conflict. Check DATABASE_URL params (pgbouncer=true & connection_limit=1).'
        )
        throw new ServiceUnavailableException(
          'Database routing issue detected (PgBouncer prepared statements). Please try again and contact support if it persists.'
        )
      }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (isPreparedStatementError && attempt < maxRetries) {
          this.logger.warn(
            `🔄 Prepared statement error detected, retrying (${attempt}/${maxRetries})`,
            {
