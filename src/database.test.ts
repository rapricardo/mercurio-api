...
describe('Database Integration', () => {
  ...
  describe('Multi-tenant Data Model', () => {
    ...
    it('should enforce tenant isolation', async () => {
      const { tenant: tenant1, workspace: workspace1 } = await testUtils.createTestTenant('Tenant 1');
      const { tenant: tenant2, workspace: workspace2 } = await testUtils.createTestTenant('Tenant 2');
      ...
      await testUtils.createTestEvent(tenant1.id, workspace1.id, visitor1.anonymousId, 'test_event_1');
      await testUtils.createTestEvent(tenant2.id, workspace2.id, visitor2.anonymousId, 'test_event_2');
  ...
  describe('Events and Sessions', () => {
    it('should create session and events with proper relationships', async () => {
      const { tenant, workspace } = await testUtils.createTestTenant();
      const visitor = await testUtils.createTestVisitor(tenant.id, workspace.id);
      const session = await testUtils.createTestSession(tenant.id, workspace.id, visitor.anonymousId);
