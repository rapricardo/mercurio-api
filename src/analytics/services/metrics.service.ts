...
export class MetricsService {
  ...
  async calculateOverviewMetrics(
    ...
    const conversionRate = currentMetrics.uniqueVisitors > 0 
  ...
  async calculateUserAnalytics(
    ...
    const identificationRate = userSummary.totalVisitors > 0
  ...
  private calculateTrend(current: number, previous: number): {
    ...
    let direction: 'up' | 'down' | 'stable' = 'stable';
    if (Math.abs(changePct) >= 5) { // 5% threshold for trend significance
      direction = changePct > 0 ? 'up' : 'down';
    }
