...
export class FunnelAnalyticsController {
  ...
  private validateAttributionAnalysisRequest(request: AttributionAnalysisRequest): void {
    ...
    if (request.attribution_models) {
      const validModels = ['first_touch', 'last_touch', 'linear', 'time_decay', 'position_based', 'data_driven', 'custom'];
      const invalidModels = request.attribution_models.filter(model => !validModels.includes(model));
    ...
    if (request.custom_model_weights) {
      ...
      if (weights.first_touch_weight < 0 || weights.first_touch_weight > 1 ||
          weights.last_touch_weight < 0 || weights.last_touch_weight > 1 ||
          weights.middle_touches_weight < 0 || weights.middle_touches_weight > 1) {
        errors.push('All custom model weights must be between 0 and 1');
      }

      const totalWeight = weights.first_touch_weight + weights.last_touch_weight + weights.middle_touches_weight;
    ...
    if (request.dimension_breakdown) {
      const validDimensions = ['utm_source', 'utm_medium', 'utm_campaign', 'device_type', 'browser', 'traffic_source', 'referrer_domain'];
      const invalidDimensions = request.dimension_breakdown.filter(dim => !validDimensions.includes(dim));
  ...
  private validateFunnelComparisonRequest(request: FunnelComparisonRequest): void {
    ...
    if (request.baseline_funnel_id && request.funnel_ids && 
    ...
    if (request.segment_dimensions) {
      const validDimensions = [
        ...
        'device_type', 'browser', 'traffic_source', 'referrer_domain'
      ];
      const invalidDimensions = request.segment_dimensions.filter(dim => !validDimensions.includes(dim));
      if (invalidDimensions.length > 0) {
    ...
    if (request.time_series_granularity && 
  ...
  async getBottlenecks(
    ...
    try {
      ...
      this.logger.error(`Bottleneck detection error for funnel ${funnelId}`, error instanceof Error ? error : new Error(String(error)), {
        tenantId: tenant.tenantId.toString(),
        workspaceId: tenant.workspaceId.toString(),
        funnelId,
      });

      this.recordAnalyticsMetrics(endpoint, Date.now() - startTime, tenant.tenantId.toString(), false, true);
  ...
  async getPathAnalysis(
    ...
    try {
      ...
      // Record successful request metrics
      this.recordAnalyticsMetrics(endpoint, Date.now() - startTime, tenant.tenantId.toString(), result.query_performance.cache_hit);
      ...
      this.logger.error(`Path analysis error for funnel ${funnelId}`, error instanceof Error ? error : new Error(String(error)), {
        tenantId: tenant.tenantId.toString(),
        workspaceId: tenant.workspaceId.toString(),
        funnelId,
      });
  ...
  async getAttributionAnalysis(
    ...
    try {
      ...
      const request: AttributionAnalysisRequest = {
        ...
        start_date: query.start_date || this.getDefaultStartDate(),
        end_date: query.end_date || this.getDefaultEndDate(),
        attribution_models: query.attribution_models || ['first_touch', 'last_touch', 'linear', 'time_decay'],
      ...
      this.logger.error(`Attribution analysis error for funnel ${funnelId}`, error instanceof Error ? error : new Error(String(error)), {
        tenantId: tenant.tenantId.toString(),
        workspaceId: tenant.workspaceId.toString(),
        funnelId,
      });
  ...
  async compareFunnels(
    ...
    try {
      ...
      this.logger.debug('Funnel comparison completed successfully', {
        ...
        statisticallySignificant: result.statistical_comparison.overall_significance.is_statistically_significant,
      ...
      this.logger.error('Funnel comparison error', error instanceof Error ? error : new Error(String(error)), {
        tenantId: tenant.tenantId.toString(),
        workspaceId: tenant.workspaceId.toString(),
        funnelIds: request.funnel_ids,
      });
  ...
  async createFunnelExport(
    ...
    try {
      ...
      this.recordAnalyticsMetrics(endpoint, Date.now() - startTime, tenant.tenantId.toString(), true, false);
      
      return result;

    } catch (error) {
      this.logger.error('Funnel export creation error', error instanceof Error ? error : new Error(String(error)), {
        tenantId: tenant.tenantId.toString(),
        workspaceId: tenant.workspaceId.toString(),
        funnelId
      });
  ...
  async getExportStatus(
    ...
    try {
      ...
      this.logger.error('Export status check error', error instanceof Error ? error : new Error(String(error)), {
        tenantId: tenant.tenantId.toString(),
        workspaceId: tenant.workspaceId.toString(),
        exportId
      });
