...
export class FunnelRealtimeService {
  ...
  async processEventForFunnels(event: EventData): Promise<void> {
    ...
    try {
      ...
      this.mercurioLogger.error('Error processing event for funnels', error instanceof Error ? error : new Error(String(error)));
  ...
  private async processEventForFunnel(
    ...
    try {
      ...
      this.mercurioLogger.error('Error processing event for specific funnel', error instanceof Error ? error : new Error(String(error)));
  ...
  private async getActiveFunnels(
    ...
    try {
      ...
      this.mercurioLogger.error('Error fetching active funnels', error instanceof Error ? error : new Error(String(error)));
      return [];
    }
  }
  ...
  private findMatchingStep(
    event: EventData,
    steps: FunnelStepMatch[]
  ): FunnelStepMatch | null {
  ...
  private async getUserFunnelState(
    ...
    try {
      ...
      this.mercurioLogger.error('Error fetching user funnel state', error instanceof Error ? error : new Error(String(error)));
      return null;
    }
  }
  ...
  private async storeUserFunnelState(
    ...
    try {
      ...
      this.mercurioLogger.error('Error storing user funnel state', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  private async updateLiveMetrics(
    ...
    try {
      ...
      this.mercurioLogger.error('Error updating live metrics', error instanceof Error ? error : new Error(String(error)));
   ...
   */
  private mapConversionStatusToStatus(conversionStatus: 'in_progress' | 'converted' | 'abandoned'): string {
