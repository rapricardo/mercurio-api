...
export class FunnelAnalyticsService {
  ...
  async getConversionAnalysis(
    ...
    try {
      ...
      const funnel = await this.funnelRepository.getFunnelById(tenantId, workspaceId, funnelId)
      ...
      const segmentAnalysis = request.include_segments
        ? await this.calculateSegmentConversions(tenantId, workspaceId, funnelId, request)
        : []

      // Get time series data if requested
      const timeSeriesData = request.include_timeseries
        ? await this.calculateConversionTimeSeries(tenantId, workspaceId, funnelId, request)
        : []
      ...
      // Cache the result
      await this.cache.set(cacheKey, result, this.cache.getTTL('conversionMetrics'))
      ...
      this.logger.error(
        'Failed to calculate conversion analysis',
  ...
  private async calculateConversionRates(
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      const conversionFromPrevious =
        previousStepUsers > 0 ? (stepCompletions / previousStepUsers) * 100 : 0

      // Calculate conversion rate from start
      const conversionFromStart = totalEntries > 0 ? (stepCompletions / totalEntries) * 100 : 0
  ...
  private async calculateSegmentConversions(
    ...
    for (const segment of deviceSegments) {
      const conversionRate =
        Math.round((segment.total_conversions / segment.total_entries) * 10000) / 100
      segments.push({
        segment_type: 'device',
  ...
  private async calculateStatisticalSignificance(
    ...
    const previousPeriod = this.calculatePreviousPeriod(request.start_date, request.end_date)

    const previousMetrics = await this.calculateConversionRates(tenantId, workspaceId, funnelId, {
      ...request,
    ...
    const pooledRate =
      (current.total_conversions + previousMetrics.overall.total_conversions) /
      (currentN + previousN)
  ...
  private async calculatePeerComparison(
    ...
    const peerMetrics = await this.analyticsRepository.getPeerFunnelMetrics(tenantId, workspaceId)
  ...
  async getDropoffAnalysis(
    ...
    try {
      ...
      const exitPaths =
        request.include_exit_paths !== false
      ...
      const biggestBottleneckStep = bottlenecks.length > 0 ? bottlenecks[0].step_order : 0
      ...
      const optimizationPotential =
        bottlenecks.length > 0
          ? Math.min(
              100,
              bottlenecks.reduce((sum, b) => sum + b.severity_score, 0) / bottlenecks.length
            )
          : 0
      ...
      const response: DropoffAnalysisResponse = {
        ...
        exit_paths: exitPaths.map((path) => ({
          ...
          exit_destinations: path.exit_destinations.map((dest) => ({
            ...dest,
            destination_type: dest.destination_type as
              | 'page'
              | 'external'
              | 'bounce'
              | 'other_funnel',
          ...
          exit_patterns: path.exit_patterns.map((pattern) => ({
            ...pattern,
            pattern_type: pattern.pattern_type as
              | 'immediate_bounce'
      ...
      // Cache the result
      await this.cache.set(cacheKey, response, this.cache.getTTL('conversionMetrics'))
      ...
      this.logger.error(
        'Failed to calculate drop-off analysis',
        error instanceof Error ? error : new Error(String(error)),
        {
          tenantId: tenantId.toString(),
          workspaceId: workspaceId.toString(),
  ...
  private generateExitTriggers(step: {
    ...
    if (velocity === 'hesitant' && avgTime > 600) {
      triggers.push({
        ...
        description:
          'Long hesitation before exit indicates unclear instructions or complex content',
  ...
  async getCohortAnalysis(
    ...
    try {
      ...
      const cohortComparison =
        request.include_comparisons !== false
      ...
      const segmentCohorts = request.segment_cohorts
        ? await this.generateSegmentCohorts(cohorts, request.segment_cohorts)
        : undefined
      ...
      this.logger.log('Cohort analysis completed', {
        ...
        avgConversionRate:
          cohorts.length > 0
            ? Math.round(
                (cohorts.reduce((sum, c) => sum + c.final_conversion_rate, 0) / cohorts.length) *
                  100
              ) / 100
            : 0,
        duration: Date.now() - startTime,
  ...
  private generateSegmentInsights(cohorts: any[], segmentType: string): string[] {
    ...
    const avgConversionRate =
      cohorts.reduce((sum, c) => sum + c.final_conversion_rate, 0) / cohorts.length
    insights.push(
      `Average ${segmentType} segment conversion rate: ${avgConversionRate.toFixed(2)}%`
    )
    ...
    insights.push(
      `Best performing ${segmentType} cohort: ${bestCohort.cohort_period} (${bestCohort.final_conversion_rate.toFixed(2)}%)`
    )
    ...
    insights.push(
      `Worst performing ${segmentType} cohort: ${worstCohort.cohort_period} (${worstCohort.final_conversion_rate.toFixed(2)}%)`
    )
  ...
  private generateCohortInsights(cohorts: any[], comparisons: any[]): any[] {
    ...
    insights.push({
      ...
      recommended_actions:
        avgConversionRate < 5
          ? [
              'Investigate low-performing cohorts',
    ...
    if (cohorts.length >= 3) {
      ...
      const trendDirection =
        Math.abs(trendChange) < 5 ? 'stable' : trendChange > 0 ? 'improving' : 'declining'
  ...
  private generateTimingInsights(
    ...
    if (overallTiming.statistics.mean_seconds > 86400) {
      // > 1 day
      insights.push({
        type: 'timing_insight',
        ...
        recommendation:
          'Consider optimizing high-friction steps and simplifying the conversion process.',
        data: {
    ...
    bottlenecks.forEach((bottleneck) => {
      insights.push({
        ...
        description:
          bottleneck.issue_type === 'slow_progression'
    ...
    if (velocityTrends.length >= 2) {
      ...
      if (decliningTrends >= 2) {
        insights.push({
          ...
          description:
            'Conversion times have been increasing in recent periods, indicating potential performance degradation.',
          recommendation:
            'Monitor for system performance issues or changes in user behavior patterns.',
          data: {
    ...
    if (segmentTiming.length > 0) {
      ...
      if (slowSegments.length > 0) {
        ...
        insights.push({
          ...
          recommendation:
            'Consider optimizing the experience for this user segment or investigating technical issues.',
          data: {
  ...
  private detectTimingBottlenecks(
    ...
    const avgTime =
      stepTiming.reduce((sum, step) => sum + step.avg_time_to_next_seconds, 0) / stepTiming.length
    const avgAbandonment =
      stepTiming.reduce((sum, step) => sum + step.abandonment_rate, 0) / stepTiming.length
    ...
    stepTiming.forEach((step) => {
      ...
      if (step.abandonment_rate > avgAbandonment * 1.5 && step.abandonment_rate > 20) {
        bottlenecks.push({
          ...
          step_label: step.step_label,
          severity:
            step.abandonment_rate > 50 ? 'high' : step.abandonment_rate > 30 ? 'medium' : 'low',
   ...
   */
  private formatUserProgressionResponse(
    rawData: any,
    ...
    const journeyHistory = journeyEvents.map((event: any, index: number) => ({
      ...
      time_from_start: Math.floor(
        (event.timestamp.getTime() - userState.enteredAt.getTime()) / 1000
      ),
      time_from_previous:
        index > 0
          ? Math.floor(
              (event.timestamp.getTime() - journeyEvents[index - 1].timestamp.getTime()) / 1000
            )
          : 0,
      event_details: {
    ...
    const stepTiming = stepTimingAverages.map((timing: any) => {
      const userTimeToReach =
        currentStep >= timing.step_order
      ...
      return {
        ...
        percentile_ranking: userTimeToReach
          ? this.calculatePercentile(userTimeToReach, avgTime)
          : null,
      }
    })
    ...
    return {
      ...
      performance_indicators: {
        ...
        time_to_conversion:
          userState.status === 'completed' && userState.completedAt
