...
export class BottleneckDetectionService {
  ...
  async detectBottlenecks(
    ...
    try {
      ...
      const rawData = await this.getRawAnalyticsData(tenantId, workspaceId, funnelId, request)
  ...
  private async analyzeBottlenecks(
    ...
    for (const currentStep of rawData.current.stepMetrics || []) {
      ...
      const dropPercentage =
        historicalStep.conversionRate > 0
          ? ((historicalStep.conversionRate - currentStep.conversionRate) /
              historicalStep.conversionRate) *
            100
          : 0

      if (
        dropPercentage >= sensitivityConfig.minDropPercentage &&
        significance.pValue <= sensitivityConfig.significanceThreshold
      ) {
      ...
      if (currentStep.avgTimeToComplete && historicalStep.avgTimeToComplete) {
        const timeIncrease =
          ((currentStep.avgTimeToComplete - historicalStep.avgTimeToComplete) /
            historicalStep.avgTimeToComplete) *
          100
  ...
  private detectSPCAnomalies(timeSeries: any[]): PerformanceAnomaly[] {
    ...
    for (let i = 0; i < timeSeries.length; i++) {
      ...
      const isAnomalous =
        point.conversionRate > upperControlLimit || point.conversionRate < lowerControlLimit
      ...
      if (isAnomalous) {
        ...
        if (anomalyStart && anomalousPoints.length >= 2) {
          ...
          const anomaly = this.createSPCAnomaly(
            anomalyStart,
            anomalousPoints,
            mean,
            standardDeviation
  ...
  private createBottleneckFromTimeStuck(
    ...
    return {
      ...
      impact_analysis: {
        lost_conversions_estimated: Math.round(
          (timeIncrease / 100) * currentStep.totalEntries * 0.1
        ),
        users_at_risk: currentStep.totalEntries,
   ...
   */
  private createConversionDropRecommendation(
