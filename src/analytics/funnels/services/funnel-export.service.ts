...
export class FunnelExportService {
  ...
  constructor(
    ...
    this.downloadBaseUrl = process.env.EXPORT_DOWNLOAD_BASE_URL || 'http://localhost:3000/downloads/exports';
  ...
  async createExport(
    ...
    const exportJob: ExportJob = {
      ...
      metadata: {
        ...
        estimated_completion: new Date(now.getTime() + sizeEstimation.estimatedDuration).toISOString(),
  ...
  async getExportStatus(exportId: string): Promise<ExportStatusResponse> {
    ...
    const progressPercent = exportJob.metadata.total_records > 0
      ? Math.round((exportJob.metadata.processed_records / exportJob.metadata.total_records) * 100)
      : 0;
  ...
  private async generateExcelFile(filePath: string, data: any): Promise<string> {
    ...
    if (data.step_metrics) {
      ...
      stepSheet.addRows(data.step_metrics.map((step: any) => ({
        step_order: step.step_order,
        step_name: step.step_name,
        unique_users: step.unique_users,
