...
export class FunnelComparisonService {
  ...
  async compareFunnels(
    ...
    try {
      ...
      const funnelData = await this.getFunnelComparisonData(
        tenantId,
        workspaceId,
        request,
      );
      ...
      const statisticalComparison = await this.performStatisticalComparison(
        funnelData,
        request.ab_test_configuration?.confidence_level || this.defaultConfiguration.default_confidence_level,
      );

      // Core metrics comparisons
      const conversionRateComparison = await this.compareConversionRates(funnelData, baselineFunnelId);
      ...
      const abTestResults = request.ab_test_configuration
        ? await this.performABTestAnalysis(funnelData, request.ab_test_configuration, baselineFunnelId)
        : undefined;
      ...
      this.logger.error('Error in funnel comparison', error instanceof Error ? error : new Error(String(error)), {
        tenantId,
        workspaceId,
        funnelIds: request.funnel_ids,
        comparisonPeriod: request.comparison_period,
  ...
  private async getFunnelComparisonData(
    ...
    for (const funnelId of request.funnel_ids) {
      try {
        ...
        const totalConversions = stepCount > 0 ? await this.analyticsRepository.getStepCompletions(
          BigInt(tenantId),
          BigInt(workspaceId),
          BigInt(funnelId),
          stepCount, // Last step
        ...
        this.logger.error(`Error getting data for funnel ${funnelId}`, error instanceof Error ? error : new Error(String(error)));
  ...
  private async performStatisticalComparison(
    ...
    return {
      ...
      multiple_comparison_correction: {
        ...
        significant_pairs_count: correctedComparisons.filter(c => c.comparison_metrics.statistical_significance).length,
      },
  ...
  private async performPairwiseComparison(
    ...
    const conversionRateLift = funnelA.overallConversionRate > 0 
    ...
    return {
      ...
      practical_assessment: {
        ...
        business_significance: this.assessBusinessSignificance(Math.abs(conversionRateDifference), Math.abs(cohensH)),
        recommendation: this.generatePairwiseRecommendation(pValue, Math.abs(cohensH), Math.abs(conversionRateDifference)),
      },
    };
  }
  ...
  private async compareTimings(
    ...
    const timingRankings = [...funnelData]
      .sort((a, b) => (a.timingData?.averageTimeToConversion || 0) - (b.timingData?.averageTimeToConversion || 0));
    ...
    for (const funnel of funnelData) {
      ...
      overallTimingMetrics[funnel.funnelId] = {
        ...
        conversion_velocity_score: this.calculateVelocityScore(funnel.timingData?.averageTimeToConversion || 0),
        rank: rank,
      };
    }
    ...
    const timeEfficiencyGap = (slowestFunnel.timingData?.averageTimeToConversion || 0) - 
  ...
  private async performABTestAnalysis(
    ...
    const businessImpact = this.assessBusinessImpact(projectedLift, pairwiseTest.comparison_metrics.statistical_significance);
    ...
    return {
      test_configuration: {
        ...
        minimum_sample_size: abTestConfig.minimum_sample_size || this.defaultConfiguration.minimum_sample_size_per_funnel,
        actual_sample_size: totalSample,
      },
      test_status: {
        is_conclusive: pairwiseTest.comparison_metrics.statistical_significance && hasSufficientSample,
      ...
      statistical_results: {
        winner: pairwiseTest.comparison_metrics.statistical_significance ? bestVariant.funnelId : null,
  ...
  private validateComparisonRequest(request: FunnelComparisonRequest): void {
    ...
    if (request.funnel_ids.length > this.defaultConfiguration.max_funnels_per_comparison) {
      throw new Error(`Maximum ${this.defaultConfiguration.max_funnels_per_comparison} funnels allowed per comparison`);
    }
  }
  ...
  private applyMultipleComparisonCorrection(
    ...
    const sortedComparisons = [...comparisons].sort((a, b) => a.comparison_metrics.p_value - b.comparison_metrics.p_value);
    ...
    for (let i = 0; i < sortedComparisons.length; i++) {
      ...
      const index = comparisons.findIndex(c => 
        c.funnel_a_id === comparison.funnel_a_id && c.funnel_b_id === comparison.funnel_b_id
      );
  ...
  private calculateEffectSizeAnalysis(funnelData: FunnelComparisonData[]) {
    ...
    const variance = conversionRates.reduce((sum, rate) => sum + Math.pow(rate - mean, 2), 0) / (conversionRates.length - 1);
    ...
    return {
      ...
      business_impact_estimate: etaSquared > 0.06 ? 'high' : etaSquared > 0.01 ? 'medium' : 'low' as any,
    };
  }

  private assessBusinessSignificance(rateDifference: number, effectSize: number): 'not_significant' | 'marginal' | 'meaningful' | 'substantial' {
  ...
  private async compareStepConversions(funnelData: FunnelComparisonData[], stepIndex: number, baseline: FunnelComparisonData): Promise<StepConversionComparison> {
    return {
  ...
  private async calculatePerformanceRanking(funnelData: FunnelComparisonData[], baseline: FunnelComparisonData): Promise<FunnelPerformanceRank[]> {
    return funnelData.map((funnel, index) => ({
      ...
      relative_performance: {
        vs_baseline_lift: baseline.funnelId === funnel.funnelId ? 0 : 
          ((funnel.overallConversionRate - baseline.overallConversionRate) / baseline.overallConversionRate) * 100,
        vs_average_lift: 0,
  ...
  private calculateConversionEfficiencyAnalysis(funnelData: FunnelComparisonData[]) {
    const sortedByEfficiency = [...funnelData].sort((a, b) => b.overallConversionRate - a.overallConversionRate);
    ...
    return {
      ...
      efficiency_gap_percentage: sortedByEfficiency[0].overallConversionRate - 
  ...
  private async compareStepDropoffs(funnelData: FunnelComparisonData[], stepIndex: number, baseline: FunnelComparisonData): Promise<StepDropOffComparison> {
    return {
      step_number: stepIndex + 1,
      step_name: `Step ${stepIndex + 1}`,
      step_dropoffs: {},
  ...
  private async compareStepTimings(funnelData: FunnelComparisonData[], stepIndex: number, baseline: FunnelComparisonData): Promise<StepTimingComparison> {
    return {
      step_number: stepIndex + 1,
      step_name: `Step ${stepIndex + 1}`,
      step_timings: {},
  ...
  private calculatePowerAnalysis(baseline: FunnelComparisonData, variant: FunnelComparisonData, abTestConfig: any) {
    const effectSize = Math.abs(variant.overallConversionRate - baseline.overallConversionRate) / 100;
  ...
  private calculateRecommendedDuration(funnelData: FunnelComparisonData[], abTestConfig: any): number {
  ...
  private assessBusinessImpact(projectedLift: number, isSignificant: boolean) {
    return {
      ...
      implementation_recommendation: isSignificant && Math.abs(projectedLift) > 5 ? 
        'implement_winner' as const : 'continue_testing' as const,
    };
  }
