...
export class FunnelConfigService {
  ...
  async createFunnel(
    ...
    try {
      ...
      await this.funnelCache.invalidateWorkspaceCache(tenantId.toString(), workspaceId.toString())
  ...
  async publishFunnel(
    ...
    if (!result) {
      throw new NotFoundException(`Funnel with ID ${funnelId} version ${version} not found`)
    }
  ...
  private validateStepOrdering(steps: any[]): void {
    ...
    for (let i = 0; i < sortedOrders.length; i++) {
      if (sortedOrders[i] !== i) {
        throw new BadRequestException('Step ordering must be consecutive starting from 0')
      }
    }
