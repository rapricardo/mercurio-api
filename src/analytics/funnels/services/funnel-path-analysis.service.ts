...
export class FunnelPathAnalysisService {
  ...
  async analyzeConversionPaths(
    ...
    try {
      ...
      const alternativePaths = request.include_alternative_paths !== false
        ? await this.detectAlternativePaths(journeyData, conversionPaths, request)
        : [];

      // Generate optimization opportunities
      const optimizations = request.include_efficiency_scoring !== false
        ? await this.generatePathOptimizations(conversionPaths, alternativePaths, journeyData)
        : [];

      // Analyze branching patterns
      const branchingAnalysis = request.include_branching_analysis !== false
      ...
      this.logger.error('Error in path analysis', error instanceof Error ? error : new Error(String(error)), {
        tenantId,
        workspaceId,
        funnelId,
        startDate: request.start_date,
  ...
  private async getUserJourneyData(
    ...
    try {
      ...
      this.logger.error('Error getting user journey data', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  private async detectAlternativePaths(
    ...
    const unconventionalJourneys = journeyData.filter(journey => 
      !this.matchesPrimaryPath(journey, primaryPaths) && 
  ...
  private async createPathComparisonMatrix(
    ...
    for (const metric of metrics) {
      ...
      comparisonMatrix.statistical_significance[metric] = 
        this.calculateStatisticalSignificance(allPaths, metric);
    ...
    comparisonMatrix.winner_analysis.overall_best_path = 
      this.determineOverallBestPath(comparisonMatrix.comparison_metrics);
  ...
  private calculatePathMetrics(journeys: UserJourney[]): PathMetrics {
    ...
    return {
      ...
      avg_completion_time: completionTimes.reduce((sum, time) => sum + time, 0) / completionTimes.length || 0,
      ...
      velocity_score: this.calculateVelocityScore(completionTimes, successful.length / journeys.length),
    };
  }
  ...
  private calculateEfficiencyScore(metrics: PathMetrics, steps: PathStep[]): number {
    ...
    return Math.round(
      (conversionScore * conversionWeight + 
       ...
       simplicityScore * simplicityWeight) * 100
    );
  }
  ...
  private extractPathSteps(journeys: UserJourney[]): PathStep[] {
    ...
    const referenceJourney = journeys.reduce((prev, current) => 
      ...
      journeys.filter(j => j.steps.length === prev.steps.length).length ? current : prev
    );
    ...
    return referenceJourney.steps.map((step, index) => ({
      ...
      bounce_rate: step.step_type === 'page' ? this.calculateBounceRate(journeys, index) : undefined,
  ...
  private calculateAvgTimeSpent(journeys: UserJourney[], stepIndex: number): number {
    const validJourneys = journeys.filter(j => 
      j.steps.length > stepIndex && j.steps[stepIndex].time_spent_seconds > 0
    );
    
    if (validJourneys.length === 0) return 0;
    
    const totalTime = validJourneys.reduce((sum, journey) => 
      sum + journey.steps[stepIndex].time_spent_seconds, 0
    );
  ...
  private calculateAlternativeExits(journeys: UserJourney[], stepIndex: number): number {
    ...
    const reachedStep = journeys.filter(j => j.steps.length > stepIndex);
    const continuedNormally = reachedStep.filter(j => 
      j.steps.length > stepIndex + 1 && 
      j.steps[stepIndex + 1].step_type !== 'exit'
    ).length;
    
    return reachedStep.length - continuedNormally;
  }

  private generateSuccessIndicators(metrics: PathMetrics, efficiencyScore: number): PathSuccessIndicator[] {
    ...
    if (metrics.avg_completion_time < 300) { // Less than 5 minutes
      indicators.push({
  ...
  private analyzeStepRemovalOpportunities(paths: ConversionPath[], journeys: UserJourney[]): PathOptimization[] {
    return [];
  }

  private analyzePathMergingOpportunities(conversionPaths: ConversionPath[], alternativePaths: AlternativePath[]): PathOptimization[] {
    return [];
  }

  private analyzeShortcutOpportunities(alternativePaths: AlternativePath[], conversionPaths: ConversionPath[]): PathOptimization[] {
    return [];
  }
  ...
  private createPathFlowDiagram(journeys: UserJourney[], decisionPoints: DecisionPoint[], mergePoints: MergePoint[]): PathFlowNode[] {
    return [];
  }
  ...
  private calculateStatisticalSignificance(paths: (ConversionPath | AlternativePath)[], metric: string) {
    return {
      p_value: 0.05,
