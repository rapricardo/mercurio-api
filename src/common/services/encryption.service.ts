...
export class EncryptionService {
  ...
  constructor(private readonly metrics?: MetricsService) {
    ...
    this.logger.log(`Encryption service initialized with key version ${this.config.currentKeyVersion}`);
  }
  ...
  generateFingerprint(data: string, type: 'email' | 'phone'): string {
    const secret = type === 'email' 
      ? this.config.emailFingerprintSecret 
      : this.config.phoneFingerprintSecret;

    // Normalize data before fingerprinting
    const normalizedData = type === 'email' 
      ? data.toLowerCase().trim()
      : data.replace(/\D/g, ''); // Remove non-digits for phone numbers
  ...
  async rotateKeys(): Promise<void> {
    ...
    this.logger.warn('Please follow the key rotation runbook in docs/guidelines/security-privacy.md');
  ...
  validateConfiguration(): boolean {
    ...
    for (const secret of requiredSecrets) {
      if (!this.config[secret as keyof EncryptionConfig] || this.config[secret as keyof EncryptionConfig] === '') {
  ...
  private generateDefaultKey(): string {
    ...
    this.logger.warn(`Generated default encryption key: ${key.substring(0, 8)}... (development only)`);
    return key;
  }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      if (!fingerprint || fingerprint.length !== 64) { // SHA256 hex = 64 chars
