...
export class MetricsService {
  ...
  getPrometheusMetrics(): string {
    ...
    addMetric(
      'mercurio_requests_success_total',
      snapshot.requests.success.value,
      'Successful HTTP requests'
    )
    addMetric(
      'mercurio_requests_errors_total',
      snapshot.requests.errors.value,
      'Failed HTTP requests'
    )
    addMetric(
      'mercurio_request_duration_ms',
      snapshot.requests.latency.p50,
      'Request duration P50 in milliseconds',
      'gauge'
    )

    // Event metrics
    addMetric(
      'mercurio_events_tracked_total',
      snapshot.events.tracked.value,
      'Total events tracked'
    )
    addMetric(
      'mercurio_events_batched_total',
      snapshot.events.batched.value,
      'Total batch events processed'
    )
    addMetric(
      'mercurio_events_identified_total',
      snapshot.events.identified.value,
      'Total identify events processed'
    )
    addMetric(
      'mercurio_events_duplicates_total',
      snapshot.events.duplicates.value,
      'Total duplicate events detected'
    )

    // Analytics metrics
    addMetric(
      'mercurio_analytics_requests_total',
    ...
    addMetric(
      'mercurio_analytics_cache_hit_rate',
      snapshot.analytics.cache_hit_rate,
      'Analytics cache hit rate',
      'gauge'
    )
    addMetric(
      'mercurio_analytics_query_duration_ms',
      snapshot.analytics.query_latency.p50,
      'Analytics query duration P50 in milliseconds',
      'gauge'
    )
    addMetric(
      'mercurio_analytics_slow_queries_total',
      snapshot.analytics.slow_queries.value,
      'Total slow analytics queries'
    )

    // API key metrics
    addMetric(
      'mercurio_apikey_validations_total',
      snapshot.apiKeys.validations.value,
      'Total API key validations'
    )
    addMetric(
      'mercurio_apikey_cache_hit_rate',
      snapshot.apiKeys.cache_hit_rate,
