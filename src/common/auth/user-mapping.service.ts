...
export class UserMappingService {
  ...
  async grantWorkspaceAccess(
    ...
    try {
      ...
      await client.userWorkspaceAccess.upsert({
        where: {
      ...
      // Only invalidate cache if not in transaction (avoid side effects during transaction)
      if (!tx) {
  ...
  async hasTenantAccess(userId: string, tenantId: bigint, requiredRole?: string): Promise<boolean> {
    const mapping = await this.getUserWorkspaceAccess(userId)
    ...
    // Check if user has access to any workspace in this tenant
    const tenantAccess = mapping.workspaceAccess.filter((access) => access.tenantId === tenantId)
    ...
    // Role hierarchy check - user needs the required role in at least one workspace
    const roleHierarchy = ['viewer', 'editor', 'admin']
    const requiredRoleIndex = roleHierarchy.indexOf(requiredRole)

    return tenantAccess.some((access) => {
      const userRoleIndex = roleHierarchy.indexOf(access.role)
      return userRoleIndex >= requiredRoleIndex
    })
  }

  async createUserProfile(user: SupabaseUser, tx?: any): Promise<boolean> {
    try {
      const client = tx || this.prisma
